...
## §128 根因:可见性证据奖励"被观测得少的地方"

§127.4 的假设:前景冲突来自多视点累积。**测了,成立,两张图各自独立成立。**

### 128.1 数

把两次 debug 跑里所有出现过的候选(203 次观测)按"该处 4 m 内有几趟"分组:

```
                  n     fg_conflict 中位    fitness 中位
  产品图  1 趟    55       0.2363            0.0180
  产品图  2 趟    47       0.5473            0.0318
  s16 图  1 趟    41       0.3135            0.0247
  s16 图  2 趟    57       0.5085            0.0293
  --------------------------------------------------------
  合并    单趟    96       0.2376
  合并    多趟   107       0.5305
```

**走过两次的地方,前景冲突是走过一次的 2.2 倍。而配准质量基本相同**
(0.018 vs 0.032、0.025 vs 0.029)—— **不是多趟的地方配得差,
是配得一样好却照样被罚。**

### 128.2 机制,一行代码

`visibility_consistency.cpp:74`:

```cpp
stored = std::min(stored, range);
```

预测深度图**对每个方位取所有累积地图点里最近的那个**。
而 `map_cloud` 是**跨趟累积**的:别的趟站在别处看到的首回波,
从当前视角看会落进同一个方位 bin,并且往往**更近**。取 min 就把预测距离拉短:

```
  signed_error = predicted - observed < -tol   →  记为 foreground conflict
  consistency  = consistent_bins / observed_bins
  evidence_log_odds = logit(consistency)
```

**这个偏差随地图在该处的厚度增长,与位姿对不对无关。**

而 `mean_visibility_evidence` 是假设比较器的**主排序键**(§125.2)。于是:

> **重定位器系统性地偏好被观测得少的地方。**

`11-50-34` 的错锁就是这条规则的直接产物:真位置走过两趟(罚 0.50),
25 m 外那个地方只走过一趟且邻居是 3 秒前的同一趟(罚 0.23),于是后者赢。

### 128.3 为什么之前所有方向都修不动它

- **改描述子(方向 B)**:描述子本来就 85/85、65/65 全对,改它只会更差 —— 实测确实更差
- **修地图重影(§126)**:配准从 0.0905 修到 0.0084,可见性证据纹丝不动 —— 实测确实没用
- **两者都没碰到主排序键本身**
...
1. **预测子图按视点筛**:只用与查询视角相容的关键帧(时间相近的一趟)建预测图
2. **分母改成可比 bin**:`consistency` 用 `common_bins` 而不是 `observed_bins`
3. **前景冲突要能被遮挡解释就不计**:同方位有更远的一致返回时不算冲突

**判据**:修完后 (a) 单趟与多趟地点的 `fg_conflict` 中位差从 2.2 倍降到 1.3 倍以内;
(b) `11-50-34` 不再 FULL 锁在 25 m 外;(c) today20 正确数**不低于 12**、
错锁**不增加**。三条同时成立才算成功。

**注意**:这个量是硬约束 3「free 空间考量融入 RHPD」的实现。
本节不是说自由空间推理没用 —— 是说**它现在的归一化方式把地图厚度当成了证据**。

### 128.5 三条修法里有一条当场就能否掉

候选 2「分母改成 `common_bins`」——**已有数据就能判它无效**:
§126.1 测到三个假设的 `observed_coverage` 是 **0.9914 / 0.9983 / 0.9413**,
即 `common_bins / observed_bins ≈ 0.99`。换分母顶多把 consistency 抬高 1%,
而单趟与多趟的差是 0.24 vs 0.53。**不做,别浪费一轮。**

留下候选 1(预测子图按视点筛)与候选 3(遮挡可解释就不计冲突)。
两者其实是同一件事的两个位置:**候选 1 在建预测图之前删点,
候选 3 在比较时容忍**。候选 1 更省算力(参与点更少),
候选 3 不依赖"哪一趟算相容"这个判断。**先做候选 3**,
因为它不需要引入新的阈值,而候选 1 需要定义视角相容性。

---

## §129 遮挡感知:诊断对了,修法太宽容,不发布

按 §128.4 实施「同方位有更远的一致返回时,把近处判为遮挡」(提交 `f5924d9`,
默认关闭)。预测深度图除每方位最近点外,再记与观测最接近的返回,打分对着后者。
不引入新阈值,沿用 `range_tolerance_m`。363 测试全过(新增两个,一正一反)。

### 129.1 控制组:新二进制、标志关闭

`11-50-34` 重跑:锁定帧 84、关键帧 176、状态相同,位姿到 15 位有效数字一致。
**不是逐位相同**(加分支后编译器重排浮点,差 1e-15~1e-17),但**行为相同**。
—— 这条要写出来,不能含糊说"一样"。
...
```
              关闭            开启
  匹配关键帧   176(错)        102(真位置)
  位姿        (-30.94,-6.10)  (-5.286,-5.660,-2.897)
  参考真值                     (-5.31, -5.64, -2.91)
  平移误差     25.3 m          0.025 m
  锁定帧       84 (8.4 s)      4
```

**25.3 米 → 2.5 厘米,第 84 帧 → 第 4 帧。§128 的诊断得到验证。**

### 129.3 判据 (a):失败,且判据本身没测到该测的东西

```
           单趟中位   多趟中位   比值
  关闭     0.2363    0.5472    2.32x
  开启     0.0149    0.0285    1.92x     判据要求 ≤1.30x
```

**失败。**但要说明:绝对值降了 16–19 倍,样本从 103 降到 21
(因为第 4 帧就锁,搜索期短了)。**两个接近零的中位数之比,
和原来 0.24 vs 0.55 的比不是同一个测量。**
写判据时我假设冲突项仍在同一量级、只是差距缩小,实际是它整体消失了。
**判据没能捕捉真正发生的事 —— 但判定不改,记为失败。**

### 129.4 判据 (c):正确数涨了,错锁翻倍 —— 决定性的失败

```
                关闭(基线)    开启
  正确              12          14
  algorithm_lock    15          20
  未锁               5           0
  超标(错 FULL)      3           6
```
...
```
  11-50-34   25.3 m 错锁  →  0.025 m 正确     ← 修好了
  11-58-53   0.125 m 正确 →  32.9 m / 42.3°   ← 新增大错锁
  11-47-53   0.445 m 正确 →  1.653 m          ← 退化
  11-47-22   未锁         →  2.904 m 锁错
  11-51-17   未锁         →  锁,roll 2.655° 超标
  11-46-29 / 11-49-55 / 11-56-10   未锁 → 正确   ← 赚到的三个
```

**5 个"不锁"全变成"锁":3 个对、2 个错。**

### 129.5 不发布,而且理由比"没达标"更硬

三条同时成立才算成功,(a) 与 (c) 都不成立,**不发布**。

但更根本的是**方向错了**:硬约束 2 是「零个错锁地点优先于成功率,
宁可不锁,不可锁错」。这个改动**把安全的"不锁"批量换成"锁"** ——
正确数 +2 是靠 algorithm_lock 从 15 涨到 20 换来的,同时错 FULL 从 3 涨到 6。
**即使正确数达标,这个交换方向本身就违反硬约束。**

### 129.6 修法为什么太宽容,以及下一版该怎么改

现在的规则是:**同方位只要有任意一个吻合返回,就原谅所有近处的点**。
于是冲突项几乎不再触发(0.015 量级),可见性证据对所有假设都给正分
(实测 +0.63 ~ +2.87,无一为负),**失去否决能力** —— 系统对什么都敢锁。

下一版要保住否决权。候选(未实施):

1. **只原谅"能被遮挡解释"的近点**:要求近点与吻合返回之间的距离
   小于某个与该方位角分辨率相称的量,而不是无条件原谅
2. **冲突与一致分开计分**:consistency 用遮挡感知,
   但 `foreground_conflict_ratio` 仍按最近点算,让它单独保留否决作用
3. **只对"多趟"方位启用**:单趟地点本来就没有这个偏差,不需要放宽

**判据不变**(三条同时成立),并**新增一条**:
`algorithm_lock` 数不得高于基线的 15 —— 直接把「宁可不锁」写进判据,
不再依赖我事后判断。

---

## §130 尺度无关的模糊门:对照精确通过,收回大半损害,仍不发布

§129.6 我写"让 `foreground_conflict_ratio` 保留否决作用"。**查代码发现它只写进
调试摘要,决策里没人读它** —— 那个否决权不存在,按它设计修法会围着一个
不产生影响的量打转。**这是本项目第五次遇到"读了、校验了、打印了,就是没接上"。**

### 130.1 真正的失效机制:固定阈值 vs 移动的工作点

```
                    关闭      开启
  consistency 中位  0.4240   0.8443
  margin 中位       0.0769   0.8232     阈值固定 0.35
  因 margin 被拒     16 帧     0 帧
```

`evidence_log_odds = logit(consistency)`,而 `d(logit)/dc = 1/(c(1-c))`。
遮挡感知把工作点从 0.42 推到 0.84,**同样的 consistency 差在那里读出大得多的
log-odds 差**,于是门从"经常开"变成"一次不开"。

**不是"太宽容",是阈值的含义随工作点漂了。**

### 130.2 收回一条上一节的推断

我曾写"遮挡感知真的降低了区分度,logit 把它藏了",依据是各取一帧比 Δconsistency。
**开启臂只有 3 帧有 ≥2 个 basin**(第 4 帧就锁了),样本不成立,
且两帧对应不同查询位姿 —— **与 §113 是同一个错**。**收回。**
区分度是真变好还是尺度变了,该数据判不了。

### 130.3 做法:把门换到不随工作点漂的量上(推导,非拟合)

```
  d(logit)/dc = 1/(c(1-c))
  基线工作点 c = 0.424  →  斜率 4.10
  0.35 log-odds  ≈  0.085 consistency
```

用 `Δconsistency < 0.085` 代替 `margin < 0.35 && ratio < 1.05`。
默认 0.0 = 沿用旧门。提交 `42f48aa`,363 测试全过。
...
**臂 4 与基线逐项相同 —— 换算忠实,臂 3 可解释。**
新门把臂 2 的损害收回大半(超标 6→4、锁定 20→18),同时保住 +2 正确。

臂 3 逐个 case:

```
  赚:  11-46-29 / 11-49-55 / 11-56-10   未锁 → 正确
       11-50-34   25.3 m 错锁 → 0.029 m 正确
  赔:  11-58-53   0.125 m 正确 → 32.9 m / 42.3°
       11-47-53   0.445 m 正确 → 未锁
       11-51-17   未锁 → roll 2.655° 超标
```
...
| | 判据 | 结果 |
|---|---|---|
| (b) | `11-50-34` 不再锁错 | **达成**(0.029 m) |
| (c) | ≥12 正确 **且** 错 FULL ≤3 | 正确 14 ✓,**错 FULL 4 ✗** |
| (d) | `algorithm_lock` ≤15 | **18 ✗** |

**(c)(d) 不成立,不发布。默认已还原。**

### 130.6 卡着的只剩一个 case,而且是另一个病

`11-58-53`(臂 3 配置,带 debug):

```
  锁定帧 4(0.4 s)      基线 2.4 s —— 快 6 倍
  锁到 kf 18,离真值 33.09 m
  候选第 1  kf 256,离真值 0.50 m      ← 描述子又是对的
  候选第 2  kf 18,fused 0.2035 vs 0.2017(差 0.9%)
  basin_sep 2.08 < 3.0  →  basin_separated 为假 → 不判模糊
  真值处 1 趟,锁到处 1 趟             ← 与多趟偏差无关
```

两条,都不是 §128 那个病:

1. **模糊门在这里空转。**`basin_separated` 要求两个顶假设相距 ≥3 m。
   这里 2.08 m,于是**两个都错且互相靠近的假设**让门直接跳过。
   门的前提是"靠得近就是同一处、不必分辨",但**同一处也可以是错的同一处**。
2. **锁得太早。**基线 2.4 s,现在 0.4 s。

### 130.7 下一步假设(未验证)

**同一类问题还剩一个没修**:`reloc_lock_log_likelihood_threshold = 2.0`
也是一个**固定阈值**,加在一个被遮挡感知抬高了工作点的证据量上。
若成立,锁定会系统性提前 —— `11-58-53` 从 2.4 s 变 0.4 s 与此相符。

**验法**:开启/关闭两种配置下,量 `cumulative_log_likelihood` 越过 2.0
所需的帧数分布。**若开启后显著变少,则该阈值需要同样的尺度无关化处理。**

另一条独立的:`basin_separated` 作为模糊门的**前置条件**是错的 ——
它让"两个都错但彼此靠近"的情形免检。改法是把它从前置条件降为一个单独的量。

**判据不变(四条同时成立)。**

---

## §131 两道契约门在这一整轮里从未被评估过

跑批脚本在 arm5 之后静默退出。追下去不是脚本的问题。

### 131.1 崩溃的位置

`aggregate_today20.py` 从 **`<case>.time.log`** 旁路文件读 RSS
(`/usr/bin/time -v` 产出),而我的跑批脚本不产这个文件。于是
`rss` 列表为空 → `max(rss)` 抛 `ValueError`。

`set -eo pipefail` 让这个崩溃杀掉整个跑批 —— **arm5 跑完就退出,没有 arm6,
也没有报错**。上一版脚本有 `|| true` 挡着,这版漏了,所以之前一直只是
"末尾一段无害的红字"。

**我从第一次评分就看见那个 traceback,当成噪声看了七八轮。**

### 131.2 真正的后果:两道门一直是空的

`result.json` 里本来就有 `performance.runtime_peak_rss_kib`,聚合器不读它。
而崩溃发生在 RSS 那行,**下一行的 p95 根本没执行到**。

修好之后重评基线(我这棵树、发布权重、同一批 20 个 case):
...
对照:产品目录自己的结果用同一评分器是 **588.2 MiB / 169.78 ms**。

**这一整轮(描述子权重三臂、遮挡感知、尺度无关门、basin 修正)
每一次 today20,这两道门都没有被评估。**

### 131.3 不能就此说"是我改坏的"

对比是有混淆的:不同的地图、我这棵树重建的 atlas、不同的二进制、
以及打过补丁的 manifest 路径(§121.2 已声明过不能当产品数引用)。
**"两道门破了"是确定的;"是谁破的"没有测。**

分离的办法:同一二进制、同一地图,只切 `reloc_visibility_occlusion_aware`
等开关,看 RSS 与 p95 是否随之变化。**这是下一步,不是现在的结论。**

### 131.4 这条的形状和 A 类缺陷一样

一道门写在那里、有阈值、有打印语句 —— **但从不执行**。
与「读了、校验了、打印了,就是没接上」是同一个家族,
只是这次缺的是"跑到那一行"。

**已修**:优先读 `result.json` 里的 `runtime_peak_rss_kib`,
旁路文件作后备;两者都没有时打印 `UNMEASURED <-- the gate did not run`
而不是静默或崩溃。**一道没跑的门必须说自己没跑。**

**未修**:`aggregate_today20.py` 在 `~/ros_ws/n3mapping_v1_closeout/`,
按约定"不入库" —— **决定契约通过与否的工具不在版本控制里**。
这本身是同一家族的问题,单独记。

---

## §132 basin 修正:诊断错了,修法赔钱,不发布

### 132.1 结果

```
  臂  遮挡  门   basin  锁定  正确  超标  未锁
  1   关   旧   关      15    12    3     5    基线
  3   开   新   关      18    14    4     2
  6   开   新   开      18    13    5     2   ← 更差
  5   关   旧   开      15    12    3     5   ← 与基线逐项相同
```

**arm5(只开 basin)与基线完全相同** —— 在基线工作点上它是空操作。
**arm6 比 arm3 少一个正确、多一个超标。**

### 132.2 赔掉的是角度,不是位置

arm6 比 arm3 多出的超标是 `11-46-29`:

```
  trans 0.076 m(很好)   yaw 4.875°(限 3.0)   roll 2.206°(限 2.0)
```

**平移一直很小,是在角度上翻的车。**arm3 里它是达标的。

### 132.3 §130.6 的诊断是错的

我认定 `11-58-53` 是被 `basin_separated` 放过的。**开了修正之后它一字未变**
(32.924 m / 42.325°)。

`ambiguous = top2_viable && competing_hypothesis && separation_too_small`,
三项都要成立。我只盯着 `basin_separated`,而 `11-58-53` 真正没触发的是
**`separation_too_small`** —— 证据把两个假设分得很开,**只是偏向了错的那个**。
**这不是门的问题,是证据的问题。**

看到 `basin_sep = 2.08 < 3.0` 就断定"门被跳过",是只读了合取式里的一项
就下结论。**三项合取,要三项都查。**

### 132.4 一个被推翻的直觉,值得单记

我推理:让门更常生效 → 只会拒掉更多锁定 → 在「宁可不锁」下必然更安全。
**错。**锁定总数一样是 18,但组成变了:**推迟一次锁定,换来的是后面
一次朝向更差的锁定。**

**"更保守的门"不等于"更安全的结果"** —— 因为被拒的那一帧之后,
证据继续累积,可能把另一个更差的假设推过线。

### 132.5 结论

**basin 修正不发布,默认保持 `false`。**机制留在代码里(零风险),
`11-58-53` 的真正原因回到"证据本身选错"这一层,与 §128 同源。

**顺带**:RSS 在各臂恒为 770.x MiB(基线 770.8、arm6 771.0),
**与本轮切换的任何机制无关** —— §131 那道破掉的内存门不是这一轮改出来的。

---

## §133 停下来对齐:与 ORB-SLAM / LIO-SAM / GLIM 的对比,以及三条设计分歧

用户叫停。此前几轮的形状已经变成"切一个开关、跑 25 分钟、看正确数",
接近纪律里禁止的按 case 调阈值。本节不含新实验,只记对齐的结论。

### 133.1 闭环:三个系统怎么做

**ORB-SLAM(2/3)**:DBoW2 词袋出候选,但关键在其后的**共视一致性** ——
候选必须在**连续若干关键帧**、在共视组层面反复出现才进几何验证;
ORB-SLAM3 进一步要求与三个共视关键帧都一致。优化走 **Essential Graph**
(生成树 + 闭环边 + 强共视边)。
**核心:验证是多帧多视角的一致性,不是单次配准得分。**

**LIO-SAM**:按里程计位姿做**半径搜索**(+ 时间间隔),ICP fitness 过线即接受,
GTSAM iSAM2 增量优化。**要害:信任里程计先验,只能找到漂移未超搜索半径的闭环。**
**n3mapping 的空间候选基本是这个路子。**

**GLIM**(信心中等,细节可能有出入):不把配准**冻结**成"相对位姿 + 协方差",
而是保留 **matching cost factor**,优化中随位姿更新**重新评估**配准代价
(VGICP + GPU)。
**这直接削弱我们建图侧那个循环依赖** ——「闭环拿单帧配已漂移的子图,
配准好不过目标的内部一致性」。传统做法在"冻结成相对位姿"那一步就把
原始信息丢了。

### 133.2 重定位:粗到精是估计的完整路径,缺的是验证

用户的模型(描述子全局粗定位 → ICP/NDT 精定位)**作为估计路径是完整的**。
缺第三段:**决定要不要相信。**

n3mapping 的失败全部在第三段。证据:

```
  锁错的地方:  fitness 0.0131   inlier 1.000
  真的地方:    fitness 0.0905
```

**精定位在错的地方跑出近乎完美的结果**,因为走廊里错的地方几何上自洽。
粗定位也没错(85/85 排第一)。**整条链是通的,通向错误答案,
而没有任何一层说"我不确定"。**

粗到精之外的方法族:多假设时序累积(我们有,但打分量有偏)、
MCL 粒子滤波(不确定性表示是我们缺的)、Cartographer 分支定界
(带保证的得分,算力要算账)、**几何互相容性**(TEASER++/Quatro/ROBIN 一系
—— 给的是"这些对应彼此不矛盾"而非"我拟合得好")、负证据/自由空间(我们有)、
**退化检测**(Zhang 的 degeneracy factor,看 Hessian 最小特征值)、结构语义约束。

### 133.3 三条设计分歧(不是实现 bug)

**(1) 系统没有"我不知道"这个输出。**
`REGION_HYPOTHESIS` 与 `FULL_6DOF_LOCKED` 之间缺一个
"此处结构性歧义,拒绝"。**走廊里多处说得通不是缺陷,是环境的事实。**
正确响应是检测到并拒绝,而不是从中挑一个。现有机制全在"挑一个"。

**(2) 排序用的量和把关用的量是同一个 —— 我认为这条最致命。**
`mean_visibility_evidence` 既是主排序键,又是唯一把关量。
**一个量干两份工,就没有独立的否决权。**
这解释了为什么三个修法全赔钱:改松了把关消失(遮挡感知:
algorithm_lock 15→20)、改紧了排序坏掉。
ORB-SLAM 的共视一致性之所以有效,**恰恰因为它和相似度分数是两个独立的东西**。

**(3) 配准得分被当成置信度。**
`fitness 0.0131 / inlier 1.000` 出现在错的地方,说明
**拟合优度不携带"是不是同一个地方"的信息**。这正是互相容性方法存在的理由。

### 133.4 三个维度里我只碰了一个

```
  正确数    ≥15/20    现 12     改了七八轮,没超过 14
  零错锁    0         现 1      定位到机制,三个修法全失败
  内存      <600 MiB  现 770.8  今天才第一次看见(§131)
  后端 p95  <1000 ms  现 2238   今天才第一次看见(§131)
```

**即使锁定全修对,契约照样过不了。**下一节查性能路径。

---

## §134 独立验证证据的五个族(框架层,module 1 review 产出)

**判断标准:验证证据不能是产生这个位姿的目标函数的函数。**

ICP `fitness` 不满足 —— 它就是 ICP 极小化的量,最优点当然低。
`inlier_ratio` 也不满足 —— 变换之后落在阈值内的计数,仍是变换的函数。
**这就是错的地方能拿到 fitness 0.0131 / inlier 1.000 的原因。**

### 134.1 可观测性 / 退化(信息矩阵最小特征值)

配准信息矩阵的最小特征值与对应特征向量:**哪个方向上位姿其实没被约束住**。
fitness 量残差,特征值量**残差对位姿的约束强度** —— 走廊里沿轴平移残差几乎不变,
所以 **fitness 可以完美而位姿在那个方向根本没定下来**。

**代价:零。**GICP 已在算信息矩阵,6×6 特征分解可忽略。

**现状:已在算、已在存、已在打印、不参与判决。**

```cpp
hyp.last_rot_info_min          = re(0);   // world_localizing.cpp:538
hyp.last_trans_info_min        = te(0);
hyp.last_rot_info_marginal_min = rm(0);
```

头文件标 "Diagnostics only",锁定时 LOG(INFO) 打印并换算成 `sigma_att_deg`。
**七个闸没有一个用它。**又一个 A 类实例,而且是最相关的那个。

### 134.2 对应关系的互相容性

若两对对应都正确,则 `|p_i-p_j| ≈ |q_i-q_j|`(刚体保距)。构造一致性图求最大相容子集。
**独立性:整个计算不用变换**,只用每朵云内部的两两距离,ICP 无法优化它。
TEASER++ / Quatro / ROBIN 这一系。

**代价:便宜。**150 个对应 → 约 1.1 万次距离比较,微秒级;最大团用贪心/谱近似。

**现状:没有。**

### 134.3 负空间 / 自由空间

"地图说这里空的,我看到回波";"地图说有墙,我看穿了"。
**独立性:用的是"没有回波"这个信息**,ICP 只做点对点匹配,从不使用它。

**代价:目前很贵**(每候选两张球面深度图,是搜索期 1.4 s 的一部分)。

**现状:有,但被误用。**自由空间栅格已降级成只否决(对);
可见性证据仍做主排序键(问题 1 的根源)。

### 134.4 结构 / 语义一致性

地板高度、墙面朝向直方图、房间拓扑。**独立性:来自场景结构统计,不来自点对残差。**

**代价:极便宜**(直方图)。

**现状:建图侧有地板高度直方图,重定位判决里完全没用。**
硬约束 3 是跨楼层无气压计 —— **地板高度是 z 上唯一的结构性判别量**,而它没进重定位。

### 134.5 跨视角 / 轨迹一致性

候选关键帧的**邻居**也要匹配(ORB-SLAM 共视一致性);
N 帧下来假设位姿序列要与里程计的**运动形状**吻合。
**独立性:用的是别的数据(邻居关键帧)和别的传感器路径(里程计)。**

**代价:邻居匹配贵,轨迹形状便宜。**

**现状:部分有**(`support_keyframe_id`、`segment_consensus_ratio`、
`evidence_motion_translation`),但没组织成独立的验证链。

### 134.6 优先级

**134.1 → 134.4 → 134.2**;134.3 不新增而是**降级**(可见性从主排序键降成只否决,
与自由空间栅格一致);134.5 暂缓,邻居匹配的算力要算账。

134.1 零新增算力、已经在算、直接对应走廊混叠这个具体失败模式 ——
**唯一要做的事是让它参与判决。**

---

## §135 模块 8 review:自由空间/可见性 —— 装反了,以及四步改法

### 135.1 同一职责,两套机制,能力不对等

| | `FreeSpaceGrid`(195) | `VisibilityConsistency`(225) |
|---|---|---|
| 表示 | 3D 占据+自由栅格,Cartographer 式射线 | 球面深度图,每方位取最近点 |
| 状态 | **occupied / free / unknown 三态** | **只有两态** |
| 覆盖 vs 矛盾 | **分开**(`known_points` / `scored_points`) | **混在一个分母**(`observed_bins`) |
| 不确定性 | **有** `stddev`(二项采样) | **无** |
| 阈值 | **不用发明** —— 差异须超过测量自身噪声 | 固定常数 0.35 / 1.05 / 2.0 |
| 角色 | **只否决**(argmax 身份比较) | **主排序键 + 四个闸** |
| 开关 | **环境变量** | config |

### 135.2 §128 那个 2.2 倍是表示能力问题,不是参数问题

深度图由地图**点**构成,点只标"占据"。于是
**"地图知道这里是空的"与"地图对这里没有意见"在该表示里不可区分**。
走过两次的地方点更多 → 取 min 拉短预测距离 → 记为矛盾,
而这个表示**没有地方存放"被射线穿过的自由空间"**。

我试的遮挡感知是在这个表示上打补丁,所以按下葫芦浮起瓢。

`FreeSpaceGrid` 头注释里的实测正好是对照:
2D 投影留下 10 m 错锁 → 改 3D 后最坏错锁 **33.1 m → 1.1 m**;
分辨率 0.15/0.20/0.25 逐 case 相同(**是高原不是调出来的点**),0.10 崩、0.30 复发。

而 `Score` 里那句注释正是我撞了三次的墙:
**"A pose over thin coverage can look uncontradicted simply because the map
never observed the region."**

### 135.3 好的组件由环境变量控制

```
  N3MAPPING_FREESPACE_SELECT     默认开
  N3MAPPING_FREESPACE_MODE       默认 veto(非 kill)
  N3MAPPING_RELOC_PERSIST        默认关
  N3MAPPING_FREESPACE_MAP_PCD    可替换栅格数据源
```

**不是 config 字段** —— 不进 `runtimeConfigCanonical`、不进 `provenance.txt`、
守卫测试覆盖不到、yaml 设不了。
**一次运行的行为取决于没有被记录的东西。**
比死配置严重:死配置什么都不做,未记录的环境变量**静默改变行为且不留痕迹**。
本轮所有跑批的 provenance 里都没有这四个值。

### 135.4 不能简单删掉一个

目的重复,能力不重复。且实测不允许直接删:
可见性现在是主排序键,去掉它排序退回累积似然,
而 `11-50-34` 按似然排会选到 **28.5 m 外**那个(似然 7.57),比现在的 25.3 m 更差。
**可见性作为排序器在做真实的工作,只是有偏。**

**关键判断:排序与把关需要的性质不同,两个组件都留,职责分开。**
排序要平滑的相对分数(可见性合适);把关要独立的绝对判断
(自由空间栅格 + 退化检测合适)。现在恰好装反了。

### 135.5 四步改法(每步单独测,判据用四条)

**Step 1 — 四个环境变量升成 config 字段。**
默认值照抄现状,零行为变化。provenance 立刻有意义,守卫测试自动覆盖。
判据:测试全过 + today20 逐位不变。

**Step 2 — 退化检测进判据。**
`last_trans_info_min` / `last_rot_info_min` 已在算/存/打印(§134.1)。
加闸:最小特征值低于阈值 → 该方向未被约束 → 拒绝。
阈值有物理含义(`sigma = 1/sqrt(λ)`,可换算成"该方向位姿标准差几米"),不是猜的。
**零新增算力。**

**Step 3 — 自由空间否决做实。**
从 `free_space_best == top1` 的 argmax 比较,改成用 `Score.value ± stddev`
做可分性检验,并跨视角累积矛盾 —— **代码注释里作者已写明这是它需要的东西**。

**Step 4 — 前两步生效后,才动可见性的表示偏差。**
届时四个闸不再挂在可见性下游。
**可检验的预测:上次遮挡感知导致 algorithm_lock 15→20,
是因为四个闸都在它下游;闸独立之后重跑同一改动,不应再出现该跳变。**
预测若不成立,说明我对因果的理解仍然是错的。

判据(四条同时成立):
(a) 单趟/多趟 `fg_conflict` 差 ≤1.3×;(b) `11-50-34` 不锁错;
(c) today20 ≥12 正确且错 FULL ≤3;(d) `algorithm_lock` ≤15。

---

## §136 模块 9 review:地图资产 IO —— 三份物化,以及惰性化的设计主张

### 136.1 更正:是三份不是四份

`reloc_map_cache_` 与 atlas 的 `global_map_` 是同一个 `shared_ptr`
(`world_localizing.cpp:1799`),不是两份。我先前说四份是错的。

常驻的三份:

1. **关键帧点云**(257 帧)—— 真值来源,闭环配准与序列化都要
2. **拼装的全局点云** —— 给自由空间栅格、给 `cropGlobalMap` 取局部目标
3. **`prepared_target_`** —— **纯粹为速度而存在**

第 3 份是大头:small_gicp 每点存 `points`+`normals`+`covs`(**4×4**),
约 192 B/点,**协方差比点本身大 5 倍**。这是 GICP 的固有成本,不是浪费。
(按结构体布局推算,未做分组件实测 —— 这本身是 136.4 的一部分。)

### 136.2 atlas 是正当的速度权衡,但形式有三个问题

搜索期是 10 候选 × 195 帧;每候选现场 `prepareTargetCloud`
(体素化+法向+协方差+建树)会是灾难。atlas 把这步搬到离线,**设计动机成立**。

**问题 1:全有或全无。**

```cpp
if (atlas loaded)  registration_target = 整张地图的 prepared target;
else               registration_target = 现场准备的局部子图;
```

只有两档,没有"按邻域惰性准备 + 有界缓存"这个中间档。
**RK3588 需要的正是中间档,而架构里不存在。**

**问题 2:它改的不只是速度,是语义。**
带 atlas 时 GICP 的收敛盆地是**整张地图**;不带时被限制在**局部子图**。
这是两个不同的算法,却藏在一个叫 "atlas / prepared" 的、读起来像纯缓存的抽象后面。

**问题 3:配准与可见性用的目标云不同。**

```cpp
evaluatePoseVisibility(submap, query_cloud, predicted_pose);   // 局部子图
matcher_.alignPrepared(*prepared_target, ...);                 // 全图(有 atlas 时)
```

同一个候选,两个证据看的目标不是同一份,却被放在一起比较和累加。
**这一条是缺陷,不是取舍。**

### 136.3 已有的数否掉了"分块加载"这条路
...
```
  ...
  serialize_ms       78.758
  verify_load_ms    434.389     从盘上读回
  verify_kdtree_ms  236.201     读回后仍要重建 KD-tree
```

**加载 434 + 重建树 236 = 670 ms > 从头算 608 ms。**

**KD-tree 没有被序列化。**atlas 省下的只是体素化+法向+协方差,
省下的时间**不够补磁盘 IO**。

**所以"把 atlas 切块按需加载"不值得做** —— 加载并不比计算快。
(保留:这是整图尺度的比值,分块后固定开销会变;
要么在子图尺度重测,要么承认这个反转足够强。)

### 136.4 设计主张:按邻域惰性计算 + LRU 有界缓存

不做分块加载,做惰性计算。一次修掉三个问题:

| 问题 | 怎么被修掉 |
|---|---|
| 全有或全无 | 缓存大小可控,与地图规模解耦 → **600 MiB 门变成可设计的** |
| 语义被悄悄改 | 目标恒为局部子图,不再分两个算法 |
| 两个证据目标不同 | 配准与可见性用**同一份**子图 |

第三条白拿:`evaluatePoseVisibility` 已经用局部子图,只要配准也用它就对齐了。

### 136.5 测什么,按顺序(第 1 步最有价值)

**1. 跑不带 atlas 的路径,量 RSS 与 p95。**零代码改动,给出**设计 2 的最坏情况**
(每候选现场准备、完全无缓存)。若上界即可接受,后三步都不用做。
障碍:评测强制要求 atlas,需先看这道检查能否参数化。

**2. 单个子图的 prepare 耗时。**7 关键帧约 6 万点;整图 608 ms,
子图应在个位数毫秒 —— 但这是外推,要实测。

**3. 候选集的帧间重叠率。**决定缓存命中率,即真实代价。
预期很高(假设跨帧存活、候选来自同一描述子排序),未测过。

**4. 单份 prepared 子图的常驻大小。**用来定缓存容量。

---

## §137 模块 7 review:配准 —— 接口是对的,问题全在边界
...
**每次配准都返回完整 6×6 信息矩阵。**§134.1 那族证据不是"可以去算",
而是**已经算好、每次返回、存在每个假设上、每次锁定都打印,没有任何一个闸读它**。

`prepareTarget` / `prepareSource` 分离是对的(一个查询试 4 个 yaw 种子,准备一次复用)。
多分辨率 `{base×2, base}` 两层 + 可选 refine 层:初值来自关键帧位姿误差不大,
两层够;yaw 的大误差靠**枚举 4 个种子**解决,不靠金字塔。分工合理。

### 137.2 问题 1:输出对了,消费错了

`fitness_score` 在结构体前面、名字最像"质量";`information` 在最后。
消费者读前者。**接口没有强迫调用方面对"这个位姿在哪个方向没被约束住"。**
病在模块 1,但模块 7 的接口形状在鼓励它。

### 137.3 问题 2:单向 ICP,而且代码里写着

`world_localizing.cpp:485`:

> **This prevents one-way ICP from dragging a valid pose onto a remote dense surface.**

单向 ICP 会被远处稠密面拽走 —— 已知。**但对策是在下游加一层可见性检查,
而不是在配准层解决**;而那层补丁本身又成了 §128 的问题源。

结构性答案是 §134.2(对应互相容性)或双向/对称配准。

### 137.4 问题 3(新假设):atlas 可能让每次配准变慢

```
  搜索期每帧:  最多 10 候选 × 4 个 yaw 种子 = 40 次 alignPrepared
  195 帧    →  最多 7800 次配准
```

`alignPrepared` 的目标云,**有 atlas 时是整张地图**。

atlas 省的是**一次性**准备开销;付的是**每次配准都在大得多的目标上**
做 KD-tree 查询与对应搜索。7800 次 × 每次多几十毫秒 = 全部预算。

**所以 atlas 这个"加速"可能是负的** —— 把 O(1) 次准备成本换成 O(7800) 次查询成本。

**可与 §136.5 第 1 步同时验证**:跑不带 atlas 的路径,
**若 p95 反而下降,假设成立** —— 那样惰性缓存方案不只省内存,而是同时更快。

### 137.5 判断

模块 7 是目前**框架最健康**的一块。问题全在边界:
上游传什么(整图 vs 子图,模块 9)、下游读什么(fitness 而非 information,模块 1)、
已知缺陷被在下游打补丁而非本层解决。
...
## §138 重定位这条线四个模块 review 完毕:合并视图

| 模块 | 框架判断 | 核心问题 |
|---|---|---|
| 1 决策 | 家族对,缺三样 | 决策由打分派生;无归一化;无"放弃"出口;`relocalize()` 1125 行 |
| 8 自由空间/可见性 | **装反了** | 好组件(三态+不确定性)只否决且受环境变量控制;差组件(两态无不确定性)在做决定 |
| 9 资产 IO | 权衡正当,形式有问题 | 全有或全无;语义随 atlas 变;两个证据看不同目标 |
| 7 配准 | **最健康** | 问题全在边界,本层不需重构 |

**一条贯穿四个模块的线索**:该用来判断的量都已经存在 ——
信息矩阵(模块 7 返回、模块 1 存了不用)、
三态自由空间与其标准差(模块 8 有、被限制成 argmax 比较)、
地板高度直方图(建图侧有、重定位不用)。
**缺的不是信息,是把信息接到判决上。**

---

## §139 模块 3 review:图优化 —— 一处更正,以及参考 GLIM 的改进方向

### 139.1 更正 §133.1:信息矩阵确实进了图

我写过"闭环边的信息矩阵是配置来的常数"。**不对。**
`createRobustNoiseModel` 在信息矩阵非零时走 `Gaussian::Information(H)`,
配置的 sigma 只是回退路径。**配准的 Hessian 确实传进了图。**

### 139.2 顺序约定:正确,但没有任何东西保护它

```
  small_gicp H              [旋转; 平移]   (由 optimizer.hpp 的 dr=head<3>/dt=tail<3>
                                            与 gicp_factor.hpp 的 J 列序确认)
    ↓ copyInformation 交换
  MatchResult::information  [平移; 旋转]   ← n3mapping 内部约定
    ↓ world_localizing: block(0,0)=平移、block(3,3)=旋转    ✓
    ↓ createRobustNoiseModel 交换回来
  GTSAM Pose3               [旋转; 平移]                     ✓
```

三处自洽。**但没有一处写下这个约定**:`EdgeInfo::information` 的注释只说
"信息矩阵(协方差逆)",`copyInformation` 无注释。
**谁在任一端"修正"一下,旋转刚度就加到平移上,而且没有任何测试会红。**

(我一度判为 bug,查证后收回。记在这里是因为过程本身说明问题:
一个需要三处同时正确的约定,靠读三个文件才能确认。)

### 139.3 好的部分

- iSAM2 增量 + 批量兜底,带 `rollbackToLastState()` / `isOptimizationHealthy()`
- 鲁棒核 Huber / Cauchy / DCS 可配
- **地板姿态因子**:从关键帧自己测的地板法向给绝对 roll/pitch,**yaw 留自由** ——
  这正是重力能说与不能说的边界,设计对

### 139.4 核心问题:用配准 Hessian 当相对位姿协方差,系统性过自信

GICP 的 `H` 衡量的是"**在当前对应关系下**,残差对位姿的约束有多强"。它不包含:

- 对应关系本身可能错(走廊里对到了另一段)
- **目标子图自己的误差**(闭环配的是已经漂移的地图)

所以它系统性低估不确定性。这是建图侧循环依赖的理论根源:

> 闭环 z 精度 0.376 m ≈ 残差跨度 0.434 m —— 配准好不过目标的内部一致性

**过自信的边把误差固化,而不是被优化掉。**

这也解释了 `loop_axis_weighting`(用 Hessian 各轴重分配)为什么实测失败:
**问题不在各轴的相对比例,在整体尺度就是错的。**
重新分配一个整体偏小的协方差,不会让它变对。

### 139.5 GLIM 的做法,以及为什么它正好解这个问题

**声明:我没有读过 GLIM 源码,以下是对其公开描述的理解,信心中等。**

传统位姿图:配准跑一次 → 冻结成"相对位姿 + 协方差" → 塞进图当二元因子。
**冻结那一步把原始信息丢了**,而且冻结的是"在那一刻、那个已漂移的目标上"的结果。

GLIM 不冻结。它把 **matching cost 本身**做成因子:因子的误差**就是**配准代价,
在**当前位姿估计**上求值。于是优化过程中位姿一动,代价重新计算、重新线性化,
对应关系也可以更新。用 VGICP(体素查找代替 kNN)让求值足够便宜,再上 GPU。

**为什么正好解我们的问题**:我们的循环依赖是"闭环配的是已漂移的目标"。
图优化一轮之后,目标**已经没那么漂了** —— 但冻结的边不知道这件事。
不冻结,循环依赖就自动被打断。

### 139.6 三档可行方案(按代价排)

**关键前提:建图是离线的。**RK3588 的算力约束压的是重定位(在线),
建图一次成型,这里有余量。
...
```
  repeat k 次:
    optimize()
    对每条闭环边,用当前优化后的位姿重新配准 → 更新 measurement 与 information
  optimize()
```

不需要新的 GTSAM 因子,不需要 GPU。代价:约 42 条闭环边 × k 轮配准。
k=2~3 时是百次量级的配准,离线完全可接受。
**这一档就能打断循环依赖**,因为第二轮配的是已经被优化过的目标。

**档 B —— 自定义 GTSAM 因子(GLIM 的 CPU 版)**

实现一个误差就是体素化 GICP 代价的因子,给解析雅可比,交给 GTSAM 自己重线性化。
这是 GLIM 去掉 GPU 的版本。工作量实打实,但是**正确的终点**。

**档 C —— 只修过自信,不修冻结**

给闭环边的协方差加一项反映"目标自身不确定性"的量(如该处的残差跨度或重访离散度)。
不改优化框架,只改边的构造。**最便宜,但不解决根本。**

### 139.7 判据(先写死)与风险

**判据**:重访 |dz| p90 从 0.4231 下降,且最大值不劣于 0.5865;
地板残差段间跨度不劣于 0.254;闭环边数不减少。四项同时成立。

**风险要写在前面**:迭代重配准可能**强化**一个错误闭环 ——
自洽的漂移会在迭代中互相印证。所以必须:
(a) 保持鲁棒核开启;
(b) 每轮记录被重配准改变超过阈值的边,若某条边在轮间反复大幅跳动,说明它不可信;
(c) **验收量不能是优化器直接约束的量**(纪律 6)——
重访判据配对的正是闭环约束的关键帧,所以主判据用**地板残差段间跨度**,
重访 p90 作辅助。

**若 p90 下降而地板残差跨度不降,判为失败** —— 那说明只是把误差挪进了闭环对,
没有真的改善一致性。

---

## §140 决定:验证要合并成一个模块,不按建图/重定位分叉

### 140.1 实测:两边各有一半独立证据,且不重叠

```
  闭环侧    完全不用自由空间/可见性         (grep 无命中)
  重定位侧  完全不用邻域共识                 (grep 无命中)
  FreeSpaceGrid 只被 world_localizing 用
```

| 独立证据 | 重定位侧 | 闭环侧 |
|---|---|---|
| 邻域共识(中位数 + MAD) | **无** | **有,energy 里权重 0.35,最大项** |
| 三态自由空间(带 stddev) | **有,但被降级成 argmax 比较** | **无** |
| 信息矩阵条件数 | 算了不用 | 算了不用 |
| 结构一致性(地板高度) | 无 | 建图侧有,验证不用 |

**不是"两边做重复的事",是"本该相同的事各做了一半,而且是不重叠的一半"。**

### 140.2 决定(用户拍板):合并,不接受按建图/重定位分叉

两边回答的是同一个问题:
**给定查询点云与一块地图区域的候选对应,有多少不依赖配准本身的证据支持它。**

合并后各自立刻拿到对方那一半:

- 重定位得到**邻域共识** —— §134.5 说它需要的东西,而且已经有一份能用的实现
- 闭环得到**三态自由空间** —— 抓"几何相似但位置错"的闭环,现在闭环侧对此毫无防线

### 140.3 真正的差异,以及为什么都不构成分模块的理由

| 差异 | 应体现为 |
|---|---|
| 时间预算(闭环离线;重定位 <5 s、p95 <1 s) | **预算参数**(验证深度可调) |
| 出错代价(错闭环被稀释;错锁不可恢复,契约要零) | **不同阈值**,同一套逻辑 |
| 场景变化(重定位跨录制,家具会动;闭环同一次录制内) | **自由空间证据的一个容忍参数** |
| 先验(闭环有位姿先验但已知有偏;重定位无先验) | 闭环侧已知自己先验有偏,"无先验"是极限情形 |
...
struct VerificationEvidence {
    // 依赖配准的,不能单独用来判断
    double fitness, inlier_ratio;
    // 独立的
    Eigen::Vector3d trans_info_eigen, rot_info_eigen;   // 条件数 / 退化
    ConsensusResult neighbourhood;                       // 中位数 + MAD
    FreeSpaceGrid::Score free_space;                     // 三态 + stddev
    StructuralMatch structure;                           // 地板高度等
};

VerificationEvidence verify(query, query_neighbourhood,
                            candidate, map, Budget budget);
```

**证据由一个模块产出,阈值由调用方按语境定。**

### 140.5 代价与风险

**主要工作量与主要风险都在收拢重复**:`LoopVerifier` / `LoopConsensusVerifier` /
`LoopSegmentConsistency` 三套都在算"邻域是否一致",口径与结构各不相同。
建图侧当前行为是这三套加 `LoopReferee` 硬规则共同产生的,
**收拢过程必须能逐 case 复现旧结果**,否则就是拿建图判据去赌重构。

**重定位的预算**:邻域共识只放在最后一道闸(对将要锁的那一个候选做一次),
不进排序 —— 1 候选 × N 邻居,而不是每帧 10 候选 × 4 yaw。

### 140.6 记一句

我前面几轮一直在**从重定位那半边证据里榨更多东西**
(改描述子权重、改可见性归一化、改模糊门、改 basin 前置条件),
而缺的那一半**就在同一个仓库的另一侧,已经写好、已经验证、还带着实测注释**。
**这是整轮 review 里最该早发现的事。**

---

## §141 模块 5 review:闭环检测,以及三个待决问题

### 141.1 结构:三条来源互斥,架构没问题

```
  RHPD 主路(SC 作辅助 + 否决)   有结果就 return,不往下走
  Scan Context 兜底              只在 RHPD 关闭/为空/无果时到达
  空间半径                       独立入口 detectSpatialCandidates
```
...
```cpp
if (config_.sc_aux_veto_enabled && sc_dist > config_.sc_aux_veto_threshold) continue;
```

**SC 不参与选择,只用来否决 RHPD 的候选。**
这正是模块 8 里自由空间栅格该有的地位 —— **同一个原则,一处对一处错**。

### 141.3 问题:`fused_score` 一个字段扛三种量纲

```
  RHPD 路:  rhpd_primary_weight·(rhpd/thresh) + sc_aux_weight·(sc/thresh)   ~[0,2]
  SC 兜底:  sc_dist 原值                                                     ~[0,1]
  空间:     dist / radius                                                    ~[0,1]
```

三条不混排,所以**现在不是活的 bug**;但字段名相同含义不同,
**任何未来合并列表的改动都会静默出错,且无测试会红**。

更实际的一条:`descriptor_score` 以权重 **0.20** 进 `LoopReferee::energy`,
而它两条路算法不同(RHPD 路是两项加权和的倒数,SC 兜底只有一项)。
裁判权重是**固定的**("fixed first-principles weights"),
于是**同一把尺子量两种归一化不同的特征**。
若某天 RHPD 大面积失效、兜底成为主力,裁判行为会**悄悄改变含义**而不报错。

### 141.4 已修复并记录在案

> previously it returned nothing at all, which is why **no keyframe below 50 was
> ever examined**.

SC 需要索引前缀、RHPD 不需要;旧代码让前者的限制吃掉后者,
**前 50 个关键帧从来没被检查过**。已修。我的 `9c3a233`(索引间隔→路程门)在同一条线上。

### 141.5 待决一:能不能只留 RHPD

**SC 可以去掉,但要等 §140 的合并做完。**
SC 是极坐标鸟瞰环-扇区,RHPD 的 Part B 就是环-高 ——
**两者测的几乎是同一件事,作为"独立否决"的独立性很弱**。
顺序:先让合并后的验证模块提供真正独立的证据(自由空间 + 邻域共识),再撤 SC。

**空间半径现在不能去掉。**它的弱点已知(只找得到漂移还小的闭环),
但它**正在做实事,恰恰因为描述子召回只有 24%** ——
`9c3a233` 就是在地图最需要闭环处把空间候选从 0 变成 11。
而裁判已正确处理它的弱点(空间候选必须有"漂移位姿产生不了的确认")。
**前提:RHPD 召回被修好,且实测不丢闭环。**

### 141.6 待决二:查询侧密度 —— 收回我自己的假设

我先前说"重定位用 5 帧聚合、闭环用单帧,所以重定位更好"。**这个解释站不住。**

```
  重定位:  查询 = 5 帧聚合(密)  vs  地图 = 单帧关键帧描述子(疏)   ← 不对称,却 85/85
  闭环:    两侧都是单帧关键帧                                      ← 对称,却只有 24%
```

**聚合假设预测反了**:若聚合是关键,不对称的重定位应该更差。

### 141.7 待决三:性能反差的真正问题,以及怎么问

**更可能的解释:两个测量的任务难度不同。**

§120 的真值是 **xy ≤1 m 且路程 ≥5 m**,把**反向穿过同一条走廊**的配对也算了进去
—— 视角完全相反,本来就难;而重定位的查询是按与建图相近的方式开过去的。

**所以 24% 可能不是"描述子差",而是"该测量包含了一批本来就该失败的配对"。**

**测法(便宜、离线、秒级)**:用现成的 `rhpd_recall_eval`,把真值按朝向差分层:
...
- 前者显著高 → 24% 是视角反转造成的,**描述子没那么差**;
  该做的是"反向重访要不要算闭环"这个产品判断
- 两者都低 → 描述子确实弱,**§122 的 Part A 尺度问题就是主线**

**这个测量决定后面所有关于描述子的工作方向**,应在动描述子之前做。

---

## §142 模块 4 review:描述子 —— 置信度门用反了,这修正 §120

### 142.1 设计意图自洽

```
  Part A   三平面投影,依赖 PCA 对齐才有意义
  Part B   环-高统计,旋转不变,不需对齐
  Aux      负空间 + 垂直 token + PCA 置信度
  距离     w_A = 0.35 + 0.65·min(conf_a, conf_b),并对 flip 取最小
```

**"对齐靠不住时少信 Part A"** —— 想法本身对。

### 142.2 但门是反着的

`confidence = (l1 - l2) / (l1 + l2)`,二维各向异性。

```
  走廊     点分布拉长   l1 >> l2  →  conf ≈ 1  →  w_A = 1.0   Part A 满权重
  空旷房间 各向同性     l1 ≈ l2   →  conf ≈ 0  →  w_A = 0.35  Part A 被压低
```

- **走廊**:主轴确实定得准(所以 conf 高),**但沿走廊的位置恰恰最不可分**,
  而 Part A 的 XY 格 4.29 m 一格,分辨不出在走廊的哪一段。
  **Part A 在它最没用的地方拿满权重。**
- **空旷房间**:主轴定不准(conf 低),**但房间的 XY 布局最有区分度**,
  Part A 只拿 0.35。

**置信度衡量"朝向定得准不准",却被当成"这个块有没有用"的代理 ——
这两件事在这里是反相关的。**

同一形状在本轮 review 里第三次出现:

| 量 | 实际衡量 | 被当成 |
|---|---|---|
| ICP fitness | 残差大小 | 是不是同一个地方 |
| 可见性 consistency | 地图在此有多厚 | 位姿对不对 |
| **PCA 置信度** | **朝向定得准不准** | **Part A 有没有用** |

### 142.3 这修正 §120 的结论

我量 Part A 分离度 **−0.6%** 得出"比随机还差"。
**那是不分条件的平均**,而设计本就规定低置信度时压低 Part A。

**正确测法是按 PCA 置信度分层,而按上面的推理,预期是:
高置信度那一层的 Part A 分离度反而更差。**

若如此,**§122.1(格子太粗)就不是主线,主线是"置信度门用反了"** ——
修法完全不同:不是改 Part A 的空间尺度,而是换一个真正衡量
"Part A 在此处有无区分力"的量当权重(如 XY 占据分布的熵、或非空格子数)。

### 142.4 一段自相矛盾的注释与死代码

```cpp
// 180° disambiguation: the half with more points becomes +X
// Convention: the denser side ... -> negative principal
// Simple rule: keep the orientation that gives MORE points on positive side
// We want +X to be the direction with fewer points
// Actually use a neutral rule: just align, accept 180° and handle in distance()
(void)pos_count; (void)neg_count;
```

五条互相矛盾的约定依次写下,最终采用第五条,前面算出的计数被显式丢弃。
最终行为正确(flip 在 `distance()` 里处理),但说明**这段的约定从未被确定下来**。

### 142.5 动描述子之前必须做的三个分层测量

都用现成的 `rhpd_recall_eval`,离线秒级:

| # | 分层维度 | 能否定什么 |
|---|---|---|
| 1 | **朝向差**(<45° vs >135°) | 前者显著高 → 24% 源于视角反转,描述子没那么差(§141.7) |
| 2 | **PCA 置信度**(高 vs 低) | 高置信层 Part A 更差 → **门用反了**,§122.1 不是主线 |
| 3 | **场景类型**(走廊 vs 开阔) | 与 2 互为印证 |

**三个都便宜,且都可能推翻当前工作方向。在它们出结果前动描述子,就是在赌。**

---

## §143 描述子结构补记(§142 讲得太散,此节重讲)
...
原始点云   ──→ Part B(512)   旋转不变,不需对齐
原始点云   ──→ Aux (173)
              拼成 3037 维
```

### 143.2 Part A:三平面投影,2352 维

```
  XY 平面: x,y ∈ [-30,30]        记的"高度"是 z
  XZ 平面: x ∈ [-30,30], z ∈ [-2,6]   记 y(横向)
  YZ 平面: y ∈ [-30,30], z ∈ [-2,6]   记 x(前后)
  每平面 14×14 格 × 4 通道 = 784,× 3 = 2352
```

记的是**东西摆在哪儿**(空间布局)。**必须先对齐** ——
不规范化朝向,同一房间从不同方向看格子内容完全不同。

### 143.3 Part B:环-高统计,512 维

```
  16 个同心环(按距离)× 8 层(按高度) = 128 单元
  每单元 4 通道: 点密度、方位角方差、平均距离、距离方差
```

**方位角只用来算方差,不进索引** —— 方差旋转不变。
所以 **Part B 天然旋转不变,不需要对齐**。
...
```
  ...
  距离   = min( W(a,b), W(a翻转180°, b) )
```

PCA 只能定主轴**方向**,定不出**朝向**(±180° 不分),所以比较时翻转再比一次取小。

### 143.5 PCA 置信度

```
  conf = (l1 - l2) / (l1 + l2)      XY 协方差的两个特征值
```

**各向异性**:点云拉得越长 conf 越接近 1,越圆越接近 0。
它衡量的是**主轴方向定得准不准**。
设计逻辑:主轴定不准 → 对齐不可靠 → Part A 建在不可靠的对齐上 → 压低 `w_A`。

**逻辑自洽,但在我们的环境里门是反的**(见 §142.2):
走廊 conf≈1 给满权重而沿轴位置最不可分;房间 conf≈0 压到 0.35 而 XY 布局最有区分度。

---

## §144 模块 2(建图主循环)与模块 10(关键帧)

### 144.1 建图主循环:一条线性闸门链

```
processFrame(mode, frame):
   1. 模式分派      LOCALIZATION / MAP_EXTENSION / MAPPING
   2. 帧有效性
   3. 里程计发散闸  前端跑飞则停止收帧
                   "每多收一帧,就是用不可用的整图替换可用的局部图"
   4. 静止启动闸    平台没真动就不建图
   5. 关键帧判定    shouldAddKeyframe,否则到此为止
   6. 是关键帧      存储 + Scan Context 描述子 + RHPD 描述子
   7. 加边          第一帧先验;否则里程计边
   8. 地板姿态因子  从本帧自己的回波测地板法向 → 绝对 roll/pitch
   9. incrementalOptimize()
```

闭环走单独的 `processPendingLoopClosures()`(**474 行,全仓库第二大函数**,
仅次于 `relocalize()` 的 1125 行 —— 两个最大的函数都在做
"要不要相信一个匹配")。

**地板姿态因子的注释第三次把独立性原则写明:**

> The only absolute attitude the graph ever sees. **Measured from this scan's own
> returns, so it is independent of the pose it constrains.**

建图侧第三次写对(前两次:闭环裁判要求"漂移位姿产生不了的确认";
SC 只否决不选择)。**重定位侧一次都没有。**

**模块 2 是全仓库结构最健康的大文件**:线性闸门链,每道闸有注释说明为何存在。

### 144.2 关键帧:很薄,但耦合很重

```cpp
shouldAddKeyframe: 位移 ≥ 1.0 m  或  转角 ≥ 0.5 rad (28.6°)
```

用 `pose_odom`(未优化)—— **正确**,关键帧决策必须因果。

### 144.3 这一块最重要的一个数

**关键帧间距 1.0 m,Part A 的 XY 格子 4.29 m。**
...
**连续四个关键帧落在同一格。**Part A 在设计上就分辨不出它们 ——
不是"分辨得不够好",是**格子比采样间距粗 4 倍**。

对照 Part B:16 环铺 30 m = **1.875 m 一环**,仍比 1 m 粗,但**比 Part A 细 2.3 倍**。

**这是 §120 里 Part B(38%)优于 Part A(26%)最直接的解释,
纯几何,不需要任何关于"信息量"的论证。**

### 144.4 另外两点

**三个子图构建器**(`buildLocalSubmap` / `buildSubmapInRootFrame` /
`buildCausalSubmapInRootFrame`)。causal 版的存在说明有场合必须只用中心帧
**之前**的关键帧 —— 这个区分必要;但三个近乎同形的实现,
是模块 9 "同一操作多份物化"的同型问题。

**没有淘汰机制**,所有关键帧点云常驻 —— 770 MiB 的第一份来源。

### 144.5 判断

关键帧间距这一个纯几何阈值(1 m / 28.6°),同时决定:

- **描述子的采样密度** → 直接决定 Part A 能不能分辨相邻位置
- 地图点云总量 → 内存
- 闭环候选粒度
- 子图空间跨度

**没有任何与"这一帧带来多少新信息"相关的判据**,而它与描述子分辨率的**耦合
没有在任何地方写出来**。
