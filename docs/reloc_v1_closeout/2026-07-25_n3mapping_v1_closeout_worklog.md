...
## §133 起:架构 review 已移出本文档

2026-08-03/04 做的逐模块架构 review(原 §133–§144,约 900 行)
**已移到 `n3mapping_architecture_review.md`**,并按模块重新组织。

移走的理由:worklog 记的是**做了什么、测到了什么**,按时间追加;
架构 review 记的是**代码是什么样**,应该按模块组织、能一口气读完。
两者混在一个文件里,从 7 月 25 日追加到 8 月 4 日、144 节,谁也读不下去。

本文档 §1–§132 保留原样,是实验记录。此后新的实验继续在这里追加。

**review 的结论摘要**(细节见那份文档):

- 重定位七道锁定闸里**四道是排序量的下游**,决策成了同义反复
- 该用来判断的量**都已存在**(信息矩阵、三态自由空间+标准差、邻域共识、
  地板高度),**缺的是接到判决上**
- 验证逻辑被分成**不重叠的两半**:闭环侧有邻域共识没有自由空间,
  重定位侧反过来 → **决定合并成一个验证模块**
- **关键帧间距 1.0 m,Part A 格子 4.29 m** —— 四个关键帧落在同一格,
  纯几何地解释了 Part B(38%)优于 Part A(26%)
- **两道契约门(RSS 770.8/600、p95 2238/1000)整轮实验从未被评估**,
  评分器在打印内存那行崩溃
- 同一个错误形状出现四次:**用一个测得准的量,代理一个它并不测量的性质**
  (fitness 代理"是不是同一地点"、可见性代理"位姿对不对"、
  PCA 置信度代理"Part A 有没有用"、`predicted_translation_norm` 代理"闭环可不可信")
