...
- `n3mapping_loop_candidate_benchmark.py`：比较当前 logged RHPD/SC-scored 候选与 LIO-SAM-style 空间候选的 GT precision / recall，用于区分“检测覆盖不足”还是“处理/优化失败”。
- `n3mapping_eval_matrix.py`：汇总多次 eval run 的 matrix。

### GLIM / LIO-SAM 参考后的候选检测结论

2026-06-20 参考 GLIM 和 LIO-SAM 后，当前判断需要从“只修 loop edge Z
处理”扩展为“候选覆盖 + 处理质量”两条线并行验证。

参考实现的关键点：

- GLIM pose-graph 版本不是 descriptor top1 直通 edge。它先按历史 submap
  的空间距离和累计行驶距离找候选，再用 GICP/VGICP registration 评估
  inlier fraction，满足 overlap/inlier 条件后才加 robust loop factor。
- GLIM global mapping 版本更进一步，用 submap overlap 和 Integrated VGICP
  matching-cost factors，把 loop 处理做成子图/因子层面的约束。
- LIO-SAM 默认回环也不是 ScanContext；它用当前估计位姿在历史 keyframe
  KD-tree 中按 radius + time gap 找候选，再把历史邻近多个 keyframe 拼成
  target submap 做 ICP，fitness 通过后才加 BetweenFactor。

新增候选基准工具：

```bash
python3 src/n3mapping/tools/n3mapping_loop_candidate_benchmark.py \
  ...
  --trajectory_est /tmp/n3mapping_kitti360_drive0005_graph_trial_diag_450_20260612_r1/trajectory_est.txt \
  --output /tmp/n3mapping_loop_candidate_benchmark_drive0005_r1 \
  --loop_translation_threshold 5 \
  --loop_yaw_threshold_deg 45 \
  --min_id_gap 20 \
  --spatial_radius_m 15 \
  --spatial_min_id_gap 50 \
  --sc_top_k 5
```

`KITTI360 drive_0005` 450-frame smoke 初步结果：

| method | pair precision | pair recall | query recall | accepted precision |
| --- | ---: | ---: | ---: | ---: |
| `n3mapping_logged` | 0.036 | 0.181 | 0.184 | 1.000 |
| `sc_rerank_logged_pool` | 0.068 | 0.174 | 0.184 | 1.000 |
| `liosam_spatial_all` | 0.237 | 1.000 | 1.000 | n/a |
| `liosam_spatial_nearest` | 0.583 | 0.316 | 1.000 | n/a |

解释：

- `n3mapping_logged` 是当前 `loop_debug.jsonl` 记录到的候选，通常是
  RHPD primary retrieval + SC auxiliary score / yaw，不等于纯 SC。
- `sc_rerank_logged_pool` 只在 logged 候选池内按 `sc_distance` 重排，
  不能当成纯 SC 全局检索结果。
- `liosam_spatial_*` 是基于 keyframe pose 的 LIO-SAM-style 空间候选
  oracle，不运行 ICP；它用于判断“如果用空间回环机会作为候选源，上界会怎样”。

这个结果说明：在该 KITTI360 smoke 上，当前 accepted loop 的 false positive
不是问题，但当前 logged 候选对 GT loop opportunity 的 query coverage 明显不足。
所以后续不应只围绕 Z edge modeling；也要评估一个 spatial/submap-overlap 候选源
是否能补足 RHPD/SC 在重复或稀疏结构里的 recall。

补充跑已有 KITTI360 multi-sequence artifact：

- `drive_0000`：`n3mapping_logged` query recall `0.211`，accepted precision
  `1.0`；`liosam_spatial_nearest` query recall `0.947`。
- `drive_0005`：`n3mapping_logged` query recall `0.184`，accepted precision
  `1.0`；`liosam_spatial_nearest` query recall `1.0`。
- `drive_0003`、`drive_0007`、`drive_0010` 在当前阈值和 smoke 截取下没有
  GT loop opportunity，因此不能用于这项 recall 判断。

当前阶段的下一条算法路线应优先验证 `spatial/submap-overlap candidate source`
能否作为 RHPD/SC 的补充候选源，然后仍用现有 ICP / edge verification 防止
false loop 进入图优化。
