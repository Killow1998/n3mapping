...
double percentile(std::vector<double> values, double quantile)
{
    ...
    const double index = std::clamp(quantile, 0.0, 1.0) * static_cast<double>(values.size() - 1);
...
SphericalDepthImage makeDepthImage(double resolution_deg)
{
    ...
    image.azimuth_bins = std::max(1, static_cast<int>(std::ceil(360.0 / resolution_deg)));
    image.elevation_bins = std::max(1, static_cast<int>(std::ceil(180.0 / resolution_deg)));
    image.ranges.assign(
...
void insertPoint(const Eigen::Vector3d& point,
                 ...
                 SphericalDepthImage* image)
{
    if (!image || !point.array().isFinite().all()) return;
    const double range = point.norm();
    if (!std::isfinite(range) || range < options.range_min_m || range > options.range_max_m) return;

    double azimuth_deg = std::atan2(point.y(), point.x()) * 180.0 / M_PI + 180.0;
    if (azimuth_deg >= 360.0) azimuth_deg -= 360.0;
    if (azimuth_deg < 0.0) azimuth_deg += 360.0;
    const double elevation_deg =
        std::atan2(point.z(), std::hypot(point.x(), point.y())) * 180.0 / M_PI + 90.0;
    const int azimuth_index = std::clamp(
        static_cast<int>(std::floor(azimuth_deg / image->resolution_deg)),
        0,
        image->azimuth_bins - 1);
    const int elevation_index = std::clamp(
        static_cast<int>(std::floor(elevation_deg / image->resolution_deg)),
        0,
        image->elevation_bins - 1);
    double& stored = image->ranges[
        static_cast<std::size_t>(elevation_index) * static_cast<std::size_t>(image->azimuth_bins) +
    ...
    const VisibilityConsistencyOptions& options)
{
    ...
    for (const auto& point : query_cloud) {
        ...
        if (std::isfinite(range) && range >= options.range_min_m && range <= options.range_max_m) {
            query_ranges.push_back(range);
        }
    }
    if (query_ranges.empty()) return result;

    const double median_query_range = percentile(query_ranges, 0.5);
    const double inferred_resolution_deg =
        std::atan2(options.range_tolerance_m, std::max(options.range_min_m, median_query_range)) *
        180.0 / M_PI;
    const double resolution_deg = std::clamp(
        inferred_resolution_deg,
        std::max(1e-3, options.min_angular_resolution_deg),
        std::max(options.min_angular_resolution_deg, options.max_angular_resolution_deg));
    ...
    for (const auto& point : map_cloud) {
        if (!pcl::isFinite(point)) continue;
        insertPoint(
            T_lidar_map * Eigen::Vector3d(point.x, point.y, point.z),
            options,
            &predicted);
    }
    ...
    for (std::size_t i = 0; i < observed.ranges.size(); ++i) {
        ...
        if (has_observation) ++result.observed_bins;
        if (has_prediction) ++result.predicted_bins;
        if (!has_observation || !has_prediction) continue;
        ++result.common_bins;
        ...
        if (absolute_error <= options.range_tolerance_m) ++result.consistent_bins;
        if (signed_error < -options.range_tolerance_m) ++result.foreground_conflict_bins;
    }

    if (result.observed_bins == 0 || result.predicted_bins == 0) return result;
    const double observed_denominator = static_cast<double>(result.observed_bins);
    result.valid = true;
    result.angular_resolution_deg = resolution_deg;
    result.observed_coverage = static_cast<double>(result.common_bins) / observed_denominator;
    result.consistency_ratio = static_cast<double>(result.consistent_bins) / observed_denominator;
    result.foreground_conflict_ratio =
        static_cast<double>(result.foreground_conflict_bins) / observed_denominator;
    // Jeffreys-prior smoothing keeps exact agreement/disagreement finite without a
    // tuned clamp. The score is the per-observation log odds that the pose explains
    // a measured first return rather than contradicting it.
