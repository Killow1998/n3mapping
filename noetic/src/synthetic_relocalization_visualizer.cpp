...
sensor_msgs::PointCloud2 makeCloudMsg(const Cloud::Ptr& cloud,
...
geometry_msgs::Point makePoint(const Eigen::Vector3d& p)
{
...
void setColor(std_msgs::ColorRGBA* color, float r, float g, float b, float a)
{
    color->r = r;
    color->g = g;
    color->b = b;
...
visualization_msgs::Marker makePoseArrow(const std::string& ns,
                                          int id,
                                          const std::string& frame_id,
                                          const ros::Time& stamp,
...
visualization_msgs::Marker makeText(const std::string& frame_id,
                                     const ros::Time& stamp,
...
visualization_msgs::Marker makeStatusSphere(const std::string& frame_id,
                                             const ros::Time& stamp,
                                             const Eigen::Isometry3d& pose,
                                             bool passed)
{
...
visualization_msgs::Marker makeErrorLines(const std::string& frame_id,
                                           const ros::Time& stamp,
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "n3mapping_synthetic_relocalization_visualizer");
    ros::V_string app_args;
    ros::removeROSArgs(argc, argv, app_args);
    ...
    ros::NodeHandle nh;
    auto global_pub = nh.advertise<sensor_msgs::PointCloud2>("/n3mapping/synthetic/global_map", 1, true);
    auto before_pub = nh.advertise<sensor_msgs::PointCloud2>("/n3mapping/synthetic/query_before", 1, true);
    auto after_pub = nh.advertise<sensor_msgs::PointCloud2>("/n3mapping/synthetic/query_after", 1, true);
    auto gt_pub = nh.advertise<sensor_msgs::PointCloud2>("/n3mapping/synthetic/query_gt", 1, true);
    auto marker_pub = nh.advertise<visualization_msgs::MarkerArray>("/n3mapping/synthetic/relocalization_markers", 1, true);
    ...
    ros::Rate rate(1.0 / std::max(0.1, options.interval_sec));

    ROS_INFO("Synthetic relocalization visualization random_seed=%u max_tests=%d interval=%.3fs query_id=%ld query_source=%s",
             seed,
             options.max_tests,
             options.interval_sec,
             options.query_id,
             options.query_source.c_str());
    ROS_INFO("RViz topics: /n3mapping/synthetic/global_map, query_before(red), query_after(green), query_gt(blue), relocalization_markers");

    int test_index = 0;
    while (ros::ok() && (options.max_tests == 0 || test_index < options.max_tests)) {
        ...
        if (!query_cloud || query_cloud->empty()) {
            ROS_WARN("Synthetic query cloud is empty after dropout for query_id=%ld", query_kf->id);
            ++test_index;
            rate.sleep();
            continue;
        }
        ...
        ROS_INFO("[%s] test=%d/%s query_id=%ld seed=%u source=%s points=%zu success=%d lock=%d matched=%ld before_t=%.3f after_t=%.3f before_z=%.3f after_z=%.3f before_yaw=%.3f after_yaw=%.3f before_rp=%.3f after_rp=%.3f",
                 passed ? "PASS" : "FAIL",
                 test_index + 1,
