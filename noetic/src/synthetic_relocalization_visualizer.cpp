...
int main(int argc, char** argv)
{
    ...
    ROS_INFO("Synthetic relocalization visualization random_seed=%u max_tests=%d interval=%.3fs query_id=%ld",
             seed,
             options.max_tests,
    ...
    while (ros::ok() && (options.max_tests == 0 || test_index < options.max_tests)) {
        ...
        ROS_INFO("[%s] test=%d/%s query_id=%ld seed=%u success=%d lock=%d matched=%ld before_t=%.3f after_t=%.3f before_yaw=%.3f after_yaw=%.3f",
                 passed ? "PASS" : "FAIL",
                 test_index + 1,
