...
protected:
     ...
     * @return 随机点云
     */
    Keyframe::PointCloudT::Ptr generateRandomCloud(std::mt19937& rng,
                                                     size_t min_points = 500,
    ...
    Keyframe::PointCloudT::Ptr generateRandomStructuredCloud(std::mt19937& rng,
        ...
        for (size_t i = 0; i < num_points; ++i) {
            ...
            cloud->points[i].y = pos_dist(rng);
            // 创建波浪形表面以增加结构
            float base_z = 0.5f * std::sin(cloud->points[i].x * 0.5f) * 
                           std::cos(cloud->points[i].y * 0.5f);
     ...
     * @return 随机变换
     */
    Eigen::Isometry3d generateRandomTransform(std::mt19937& rng,
                                               double max_trans = 1.0,
                                               double max_rot = 0.3) {
        ...
        transform.translation() = Eigen::Vector3d(trans_dist(rng), 
                                                   trans_dist(rng), 
                                                   trans_dist(rng));
     ...
     * T1 * T2 应该接近单位矩阵
     */
    bool areInverseTransforms(const Eigen::Isometry3d& T1,
                               const Eigen::Isometry3d& T2,
                               double trans_tol = 0.3,
                               double rot_tol = 0.15) {
