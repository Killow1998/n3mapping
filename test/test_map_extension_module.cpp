...
/**
 * @brief MapExtensionModule 单元测试
 * 
 * 测试地图续建功能
 * Requirements: 12.1, 12.2, 12.3, 12.4, 12.5, 12.6, 12.7
 */
class MapExtensionModuleTest : public ::testing::Test {
protected:
    void SetUp() override {
        // 初始化配置
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr generateCorridorCloud(
        const Eigen::Isometry3d& pose, double corridor_width = 3.0) {
 ...
 * @brief 测试初始状态
 */
TEST_F(MapExtensionModuleTest, InitialState) {
    KeyframeManager kf_manager(config_);
    LoopDetector loop_detector(config_);
    ...
    EXPECT_EQ(extension.getState(), MapExtensionState::NOT_INITIALIZED);
 ...
 * Requirements: 12.1
 */
TEST_F(MapExtensionModuleTest, LoadExistingMap) {
    // 创建测试地图
    ...
    EXPECT_EQ(extension.getState(), MapExtensionState::MAP_LOADED);
 ...
 * @brief 测试加载不存在的地图
 */
TEST_F(MapExtensionModuleTest, LoadNonExistentMap) {
    KeyframeManager kf_manager(config_);
    LoopDetector loop_detector(config_);
    PointCloudMatcher matcher(config_);
    GraphOptimizer optimizer(config_);
 ...
 * Requirements: 12.3
 */
TEST_F(MapExtensionModuleTest, KeyframeIdContinuity) {
    // 创建测试地图
    std::string map_file = createTestMap(5);
 ...
 * @brief 测试重置功能
 */
TEST_F(MapExtensionModuleTest, Reset) {
 ...
 */
TEST_F(MapExtensionModuleTest, IsFromOriginalMap) {
    std::string map_file = createTestMap(5);
 ...
 */
TEST_F(MapExtensionModuleTest, SaveExtendedMap) {
    std::string map_file = createTestMap(3);
