...
TEST(VisibilityConsistencyTest, PredictedForegroundContradictsObservedFreeSpace)
{
    const auto query = makeFourRayCloud(5.0);
...
TEST(VisibilityConsistencyTest, SymmetricObservationProvidesEqualPoseEvidence)
{
    ...
    const auto rotated = evaluateVisibilityConsistency(
        symmetric, symmetric, quarter_turn);
