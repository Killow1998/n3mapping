 ...
 */
class MapExtensionModulePBTTest : public ::testing::Test {
protected:
    void SetUp() override {
    ...
    std::pair<std::string, int64_t> createRandomTestMap() {
        ...
        std::string map_file = config_.map_save_path + "/random_map_" + 
                               std::to_string(rng_()) + ".pbstream";
 ...
 * **Validates: Requirements 12.3, 12.7**
 */
TEST_F(MapExtensionModulePBTTest, Property9_KeyframeIdContinuity) {
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
        ...
        GraphOptimizer optimizer(config_);
        MapSerializer serializer(config_);
        RelocalizationModule relocalization(config_, kf_manager, loop_detector, matcher);
        
        MapExtensionModule extension(config_, kf_manager, loop_detector, matcher,
        ...
        EXPECT_EQ(loaded_max_id, original_max_id)
            << "Iteration " << iter << ": Max ID mismatch after loading";
        ...
        for (size_t i = 0; i < new_ids.size(); ++i) {
            ...
            EXPECT_EQ(new_ids[i], expected_id)
                << "Iteration " << iter << ", new keyframe " << i 
                << ": Expected ID " << expected_id << ", got " << new_ids[i];
        }
 ...
 */
TEST_F(MapExtensionModulePBTTest, OriginalMapIntegrity) {
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
        ...
        RelocalizationModule relocalization2(config_, kf_manager2, loop_detector2, matcher2);
        
        MapExtensionModule extension(config_, kf_manager2, loop_detector2, matcher2,
                                      optimizer2, serializer2, relocalization2);
        ...
        // 保存扩展地图
        std::string extended_file = config_.map_save_path + "/extended_" + 
                                    std::to_string(iter) + ".pbstream";
        ...
        for (int64_t id = 0; id <= original_max_id; ++id) {
            ...
            EXPECT_NE(kf, nullptr)
                << "Iteration " << iter << ": Original keyframe " << id << " missing";
        }
        
        // 验证总数增加
        EXPECT_EQ(kf_manager3.size(), original_count + 3)
            << "Iteration " << iter << ": Keyframe count mismatch";
 ...
 */
TEST_F(MapExtensionModulePBTTest, IsFromOriginalMapCorrectness) {
    constexpr int NUM_ITERATIONS = 20;
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
        ...
        // 验证所有原始 ID 返回 true
        for (int64_t id = 0; id <= original_max_id; ++id) {
            EXPECT_TRUE(extension.isFromOriginalMap(id))
                << "Iteration " << iter << ": ID " << id << " should be from original map";
        }
